      ...
      <TableBody>
        {vehicles.map((vehicle) => (
          <TableRow key={vehicle.id}>
            ...
            <TableCell className="hidden lg:table-cell">{vehicle.mileage.toLocaleString()} km</TableCell>
            <TableCell>
              <Badge variant="outline" className={cn({
                "border-green-500 text-green-600": vehicle.status === "Available",
                "border-yellow-500 text-yellow-600": vehicle.status === "Incoming",
                "border-gray-500 text-gray-600": vehicle.status === "Sold",
              })}>{vehicle.status}</Badge>
            </TableCell>
            <TableCell className="hidden lg:table-cell">
              <Badge variant={vehicle.inspectionStatus === "Passed" ? "default" : "secondary"} className={cn({"bg-green-100 text-green-800": vehicle.inspectionStatus === "Passed", "bg-yellow-100 text-yellow-800": vehicle.inspectionStatus === "Pending", "bg-red-100 text-red-800": vehicle.inspectionStatus === "Failed"})}>
